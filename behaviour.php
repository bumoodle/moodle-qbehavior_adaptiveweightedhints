 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qbehaviour_adaptiveweightedhints extends qbehaviour_adaptiveweighted
{

    /**
     * Specify the data that we expect from a form submission. 
     * In this case, we expect the value of the submit and request hint buttons. 
     * (We ignore the "save" button, as Moodle considers "save" the default.)
     */
    public function get_expected_data() 
    {
        //if the question is active, indicate that we expect submit and hint
        if ($this->qa->get_state()->is_active()) 
            return array('submit' => PARAM_BOOL, 'hint' => PARAM_BOOL);

        //otherweise, delegate to the base class
        return parent::get_expected_data();
    }

    /**
     * Processes any form-submission action.
     */
    public function process_action(question_attempt_pending_step $pendingstep) 
    {

        //handle comment/override actions
        ...
        else if ($pendingstep->has_behaviour_var('hint'))
            return $this->process_hint_request($pendingstep);    

        //all other actions are considered save requests
        else 
    ...
    /**
     * Process a hint request, updating the internal hint counter.
     */
    public function process_hint_request(question_attempt_pending_step $pendingstep)
    {
        //first, make sure we've saved the user's current input
        $status = $this->process_save($pendingstep);

        //get the maximum hint number
        $hints_available = count($this->question->hints);

        //get the most recent hint count, plus the currently requested hint
        $hint_count = $this->qa->get_last_behaviour_var('_hints', 0) + 1;

        //if the user has requested a hint which doesn't exist, abort
        if($hint_count > $hints_available)
            return question_attempt::KEEP;

        //update the hint count in the new behaviour variable
        $pendingstep->set_behaviour_var('_hints', $hint_count);
    ...
    /**
     *  Get the most appropriate hint for the given state.
     */
    public function get_applicable_hint()
    {
        ...
        {
            //get the number of the most appropriate hint
            $hint_number =  min($hint_count - 1, $hints_available);

            //and return that hint
            return $this->question->get_hint($hint_number, $this->qa);
        }
    }
